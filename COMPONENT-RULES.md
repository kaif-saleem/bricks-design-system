...
- Icon size increased 2026-08-07 (seventh revision, same day), per the designer's direction to make the icon look more aligned with the rest of the component: both Chevron and Leading Icon moved from Size=16 to Size=20 on the same Format=Stroke/Weight=Regular icon component set (the library also offers 18 and 24). This is a variant switch on the existing library instances, not a manual resize or a new icon swap. Header height auto-hugs to the larger icon (46px Web, 44px Mobile, up from ~44/45). Applied uniformly to both Device values.

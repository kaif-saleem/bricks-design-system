...
- Known non-compliance: both the chevron (`lucide:chevron-right`, rotated -90deg when open) and the leading icon (`lucide:file-text`) are placeholder Lucide vectors, not instance swaps from the Iconography (Bricks) library, because the exact component names could not be looked up without switching the frontmost Figma file away from Bricks Design System (Rules §3). Same category of gap as the deleted Checkbox test build. Must be swapped before this can move to production.

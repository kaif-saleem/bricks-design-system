...
- Anatomy: Container (vertical Auto Layout, no fill) > Header (horizontal Auto Layout, padding `s`/`m`, gap `s`) > Leading Icon (optional) + Title + Chevron (always visible) > Content Panel (only present when Open, padding `none`/`m`) > Divider (1px, always shown on every variant, not a property).
...
- Content Panel subtext, superseded 2026-08-07 (sixth revision, same day), per the designer's direction ("subtext 14px"): Web's Content Panel body text moved from `Web/Body/default_regular` (16px) to `Web/Body/small_regular` (14px), a real named text style, not a raw size override. Mobile was already `Mobile/Body/default_regular` at 14px, so it needed no change at that point. Both platforms rendered the subtext at 14px.
- Mobile subtext further reduced 2026-08-07 (ninth revision, same day), per the designer's direction that Mobile's subtext specifically should be 12px: moved Mobile's Content Panel body text from `Mobile/Body/default_regular` (14px) to `Mobile/Body/small_regular` (12px), again a real named text style. Web is unaffected and stays at `Web/Body/small_regular` (14px). Verified the change propagated to the QA Grid instances and re-ran the token-binding audit (zero issues).
- Icon instance swap resolved 2026-08-07: the Rules §3 single-file restriction blocked looking up icon names directly, so the designer dragged real Iconography (Bricks) instances onto the Accordion page from the Assets panel (with Bricks Design System already frontmost, no other file touched). Read their `mainComponent` keys and swapped in via `figma.importComponentByKeyAsync`: Chevron is `CaretDown` when Collapsed and `CaretUp` when Open, since the library publishes each caret direction as its own component rather than one rotatable icon. Leading Icon is `Files`. Both are live library instances, not detached, per Rulebook §9 instance safety; their internal fills/strokes are the library's own responsibility, not rebound here.
- Icon size increased 2026-08-07 (seventh revision, same day), per the designer's direction to make the icon look more aligned with the rest of the component. First attempt switched both Chevron and Leading Icon to the library's native Size=20 variant, but this Iconography component set scales stroke weight together with size (16px stroke 1, 18px stroke 1.125, 20px stroke 1.25, 24px stroke 1.5) -- confirmed by inspecting all four Size variants directly, not assumed. The designer then clarified the stroke weight itself must not change, only the overall icon size. Corrected 2026-08-07 (eighth revision, same day): reverted both icons to the Size=16 variant (stroke weight 1, the original), then resized each icon instance's frame to 20x20 directly (`instance.resize(20, 20)`), rather than switching the Size variant. Verified empirically via pixel-level measurement of rendered screenshots (not assumed) that this keeps the stroke's absolute rendered thickness the same as the original 16px icon while the overall glyph grows, versus the native Size=20 variant which renders visibly thicker. Header height auto-hugs to the larger icon (46px Web, 44px Mobile). Applied uniformly to both Device values and confirmed it propagated correctly to the QA Grid instances.
- Icon size split by Device 2026-08-07 (tenth revision, same day), per the designer's direction that Mobile's icon size should start at 16: reverted both Chevron and Leading Icon on the two Mobile variants back to their native 16x16 frame size (`instance.resize(16, 16)`), undoing the Size=16-resized-to-20 treatment for Mobile only. Web is unaffected and stays at 20x20. Header heights: Web 46px, Mobile 44px. Confirmed it propagated to the QA Grid's Mobile rows and that Web's grid rows stayed at 20px.
...
- QA Grid added 2026-08-07 (node 2459:7939, page "Accordion", named "Accordion - QA Grid"): a reference grid of live instances of the built component set, grouped by the two real Variant axes (Device, Expansion) as rows, then every `Show Icon` x `Show Divider` boolean combination as one labeled card per row. No new variant or property was added to the component to build this. Confirmed empirically on a real instance (not assumed) that `Show Icon` and `Show Divider` are fully independent — neither hides nor overrides the other — so all 16 combinations render distinctly. Matches the documentation page's visual language (same divider color, same text styles). The component set and its documentation frame were not touched; verified by direct position/size/child-count comparison before and after.
- Properties simplified 2026-08-07 (fifth revision, same day), per the designer's explicit final property list ("only these properties: Device, Expansion, and Show Icon"): removed the `Show Divider` boolean property entirely. The Divider layer's `componentPropertyReferences` was cleared and its `visible` set permanently true on all 4 variants, so the divider now always shows, on the designer's instruction ("divider to be there for all states"). Device and the `Collapsed`/`Open` naming stay as they were, per the designer's confirmation. Rebuilt the QA Grid (still node 2459:7939, same name) down from 16 to 8 cards (4 variants x Show Icon only, no more Divider axis) and removed the now-stale `showDivider` row from the Documentation frame's Component API table; the Specs and Color Tokens rows describing the divider's own look were left as-is since they're still accurate, with a note added that it is no longer a toggleable property.

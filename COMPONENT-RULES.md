...
- Documentation format, superseded 2026-08-07, with Shashwat's/Sreejita's sign-off obtained for this specific change: the Documentation frame on the Toggle page was rebuilt to match production Button's actual style (title in `purple/700` bold, section header + grey subtitle, light-grey-header data tables, divider rules between sections, closing checkmark/cross usage examples) instead of the plain Rulebook §13 prose template used in the original build. Same content as before (when to use, anatomy, properties, sizes, states, behaviors, color tokens, accessibility, known gaps, Material Design 3 reference, usage), restructured only. Nothing on the `Toggle` component set (node 1729:82), Prototype Demo, iPhone preview, or QA Grid was touched.

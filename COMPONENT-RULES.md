...
- Anatomy: Container (vertical Auto Layout) > Header (horizontal Auto Layout, padding `s`/`m`, gap `s`) > Title + Icon (icon trailing) > Content Panel (only present when Expanded, padding `none`/`m`) > Divider (1px, boolean-controlled).

...
| CTA (test) | do-not-use | Test | Device x State | 10 | Prior Test page/set (node 1488:35) deleted between sessions; superseded by the row below |
| CTA (test, Mobile) | do-not-use | Test | State | 5 | Test page recreated (2026-07-17, node 1491:118), exception to no-test-pages rule per Shashwat; Mobile only, CTA text style, fully bound; throwaway, delete when done reviewing |
...
| OTPField | do-not-use | OTP field | State | 3 | COMPONENT-RULES.md § OTPField; Documentation frame on the OTP field page (test build, node 2243:1229, real component set with 3 states, detached from the shared Input Fields library component to allow token binding, no live library link, mixes local and published-library color tokens; awaiting Shashwat's review) |
